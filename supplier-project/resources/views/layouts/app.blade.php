        ...
        <nav class="sb-nav">
            ...
            @auth
                @if(auth()->user()->isAdmin())
                    ...
                    @php
                        $pendingDevices = \App\Models\DeviceAuthRequest::where('status','pending')->count();
                    @endphp
                    ...
                    <a href="{{ route('device-approvals.index') }}" wire:navigate
                        class="nav-link {{ request()->routeIs('device-approvals.index') ? 'active' : '' }}">
                        <svg class="nav-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <rect x="5" y="2" width="14" height="20" rx="2" stroke-linecap="round"/>
                            <path d="M9 7h6M9 11h6M9 15h4" stroke-linecap="round"/>
                            <circle cx="18" cy="18" r="4" fill="{{ $pendingDevices > 0 ? '#ef4444' : '#22c55e' }}" stroke="white" stroke-width="1.5"/>
                        </svg>
                        Device Approvals
                        @if($pendingDevices > 0)
                            <span class="nav-badge">{{ $pendingDevices }}</span>
                        @endif
                    </a>
    ...
    <div id="main-wrap">
        ...
        <header id="top-bar">
            <span class="top-title">
                ...
                @elseif(request()->routeIs('roles.index'))      Role Management
                @elseif(request()->routeIs('device-approvals.index')) Device Approvals
                @else Dashboard
                @endif
            </span>
